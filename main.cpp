...
int main() {
  ...
  //  std::endl; Model::evaluate(weight_bias, data_test);
  ...
  auto start = std::chrono::high_resolution_clock::now();
  Model::evaluate(weight_bias, data_test);
  Model::training_multiple_threads(weight_bias, data, false, 56, 10,
                                   number_of_epochs);
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> duration = end - start;
  std::cout << "Execution time: " << duration.count() << " seconds"
  ...
  //  auto start = std::chrono::high_resolution_clock::now();
  //  Model::evaluate(weight_bias, data_test);
  //  Model::training_multiple_threads(weight_bias, data, true, 56, 10,
  //  number_of_epochs); auto end = std::chrono::high_resolution_clock::now();
  //  std::chrono::duration<double> duration = end - start;
  //  std::cout << "Execution time: " << duration.count() << " seconds" <<
  //  std::endl; Model::evaluate(weight_bias, data_test);
