...
void Model::matmul(Tensor* input, Tensor* output, Tensor* result){
    if (input->shape.size() != 2 || output->shape.size() != 2 || result->shape.size() != 2) {
        throw std::invalid_argument("All tensors must be 2D for matrix multiplication");
    }

    if (input->shape[1] != output->shape[0]) {
        throw std::invalid_argument("Incompatible dimensions: input columns must match output rows");
    }


    if (result->shape[0] != input->shape[0] || result->shape[1] != output->shape[1]) {
...
void Model::matvec_transposed(Tensor* matrix, Tensor* vector, Tensor* result) {
    if (matrix->shape.size() != 2) {
        throw std::invalid_argument("Matrix must be 2D for matrix-vector multiplication");
    }

    if (vector->shape.size() != 1) {
        throw std::invalid_argument("Vector must be 1D for matrix-vector multiplication");
    }

    if (result->shape.size() != 1) {
        throw std::invalid_argument("Result must be 1D for matrix-vector multiplication");
    }
...
int Model::forward_pass(Tensor* input, vector<Tensor*>& stored_result, vector<Tensor*>& weight_bias) {
...
void Model::backward_pass(Tensor* input,  vector<Tensor*>& stored_result, vector<Tensor*>& weight_bias, vector<Tensor*>& weight_bias_d,  Tensor* labels) {

    for (int i = 0; i < 10; i++) {
        weight_bias_d[weight_bias_d.size() - 1]->at(i) = stored_result[stored_result.size() - 1]->at(i) - labels->at(i);
    }
    for (int i = 0 ; i < 10; i++) {
        for (int j = 0 ; j < 10; j++) {
            weight_bias_d[weight_bias_d.size() - 2]->at(i, j) = weight_bias_d[weight_bias_d.size() - 1]->at(j) * stored_result[stored_result.size() - 2]->at(i);
            weight_bias_d[weight_bias_d.size() - 3]->at(i) += weight_bias_d[weight_bias_d.size() - 1]->at(j) * weight_bias[weight_bias_d.size() - 2]->at(i, j);
        }
    }
    for (int i = 0; i < 64; i++) {
        weight_bias_d[weight_bias_d.size() - 3]->at(i) *= dReLU(stored_result[stored_result.size() - 4]->at(i));
    }

    for (int i = 0 ; i < 128; i++) {
        for (int j = 0 ; j < 64; j++) {
            weight_bias_d[weight_bias_d.size() - 4]->at(i, j) = weight_bias_d[weight_bias_d.size() - 3]->at(j) * stored_result[stored_result.size() - 5]->at(i);
            weight_bias_d[weight_bias_d.size() - 5]->at(i) += weight_bias_d[weight_bias_d.size() - 3]->at(j) * weight_bias[weight_bias_d.size() - 4]->at(i, j);
        }
    }
    for (int i = 0; i < 128; i++) {
        weight_bias_d[weight_bias_d.size() - 5]->at(i) *= dReLU(stored_result[stored_result.size() - 6]->at(i));
    }
    for (int i = 0 ; i < 784; i++) {
        for (int j = 0 ; j < 128; j++) {
            weight_bias_d[weight_bias_d.size() - 6]->at(i, j) = weight_bias_d[weight_bias_d.size() - 5]->at(j) * input->at(i);
        }
    }
}
void Model::training(vector<Tensor*>& weight_bias,  Pack* data, int batch_size) {
            ...
            for (int i = 0; i < data->train_data.size(); i++) {
                ...
                backward_pass(data->train_data[i], calculations, weight_bias, weight_bias_d, data->train_label[i]);
...
void Model::add_weight_bias(vector<Tensor*>& weight_bias_cumulative_d, vector<Tensor*>& weight_bias_d){
...
float Model::evaluate(vector<Tensor*>& weight_bias, Pack* data){
    ...
    for (int i = 0; i < data->train_data.size(); i++) {
    correct += data->train_label[i]->at(forward_pass(data->train_data[i], calculations, weight_bias));
    }
    cout << "Efficiency " << ((float)correct / (float)data->train_data.size()) * 100<<  "%" << endl;
    return (float)correct / (float)data->train_data.size();
  }
void Model::update_gradient(vector<Tensor*>& weight_bias, vector<Tensor*>& weight_bias_mini_batch_d, int batch_size){
    for (int i = 0; i < weight_bias_mini_batch_d.size(); i++) {
        for (int j = 0; j < weight_bias_mini_batch_d[i]->size ; j++) {
            weight_bias[i]->storage[j] -=  learning_rate * weight_bias_mini_batch_d[i]->storage[j] / (float)batch_size;
        }
    }
  }
void Model::update_gradient_adam(vector<Tensor*>& weight_bias, vector<Tensor*>& weight_bias_mini_batch_d, vector<vector<Tensor*>*>* moments, int batch_size){
    for (int i = 0; i < (*moments)[0]->size(); i++) {
        for (int j = 0; j < (*(*moments)[0])[i]->size ; j++) {
            float gradient = weight_bias_mini_batch_d[i]->storage[j] / (float)batch_size;

            // Update first moment (m)
            (*(*moments)[0])[i]->storage[j] = (*(*moments)[0])[i]->storage[j] * first_moment_decay +
                                             (1 - first_moment_decay) * gradient;

            // Update second moment (v) - with gradient squared
            (*(*moments)[1])[i]->storage[j] = (*(*moments)[1])[i]->storage[j] * second_moment_decay +
                                             (1 - second_moment_decay) * gradient * gradient;

            // Apply bias correction and update weights
            float m_corrected = (*(*moments)[0])[i]->storage[j] / (1 - pow(first_moment_decay, t_index));
            float v_corrected = (*(*moments)[1])[i]->storage[j] / (1 - pow(second_moment_decay, t_index));

            weight_bias[i]->storage[j] -= learning_rate * m_corrected / (sqrt(v_corrected) + epsilon);
        }
    }
    t_index += 1;
...
void Model::training_multiple_threads(vector<Tensor*>& weight_bias, Pack* data, bool adam, int mini_batch_size, int num_of_threads, int epoch){
   ...
   if(adam){
    ...
    for(int k = 0; k < epoch; k++){
        ...
        for(int i = 0; i < no_of_mini_batches - 5; i++) {
            ...
            int counter = 0;
            for(int j = i * mini_batch_size; j < (i + 1) * (mini_batch_size); j = j + no_of_dp_per_thread) {
              if(counter / 3 == (num_of_threads - 1)){
                  pool.enqueue(processing_per_thread, weight_bias, store[counter], store[counter + 1], store[counter + 2], data, j, (i + 1) * (mini_batch_size) - 1);
                  continue;
              }
              pool.enqueue(processing_per_thread, weight_bias, store[counter], store[counter + 1], store[counter + 2], data, j, j + no_of_dp_per_thread - 1);
              counter += 3;
            }
            ...
            if(adam){
              update_gradient_adam(weight_bias, weight_bias_mini_batch_d, adam_data, mini_batch_size);
              } else {
...
void Model::processing_per_thread(vector<Tensor*>& weight_bias, vector<Tensor*>* weight_bias_d, vector<Tensor*>* calculations, vector<Tensor*>* weight_bias_temp_d, Pack* data, int start_index, int end_index){
                for (int i = start_index; i <= end_index; i++) {
                    ...
                    forward_pass(data->train_data[i], *calculations, weight_bias);
                    backward_pass(data->train_data[i], *calculations, weight_bias, *weight_bias_temp_d, data->train_label[i]);
                    add_weight_bias(*weight_bias_d, *weight_bias_temp_d);
                }
  }

void Model::create_calculaitons_computetions_matrix(vector<vector<Tensor*>*>& store, int num){
  for(int i = 0; i < num; i++){
